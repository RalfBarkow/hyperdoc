...
## Priority: Plan and Implement Through HyperDoc-Native Editable Objects

When the user adds new wants, priorities, or design constraints for a feature
cluster, do not leave them as chat-only prose. Materialize them first as
inspectable/editable HyperDoc artifacts so planning happens inside HyperDoc.

Prefer this order when the user is steering product/design work:

1. create or update the HyperDoc page/chunk/topic that records the wants and
   priorities,
2. make the relevant content editable in HyperDoc (for example markdown-page,
   frame/plugin chunk, state-model view),
3. link claims in that artifact to source code/tests through inspectable
   relations,
4. then update runtime behavior, chrome, JS/CSS, and tests.

If the user explicitly says the priorities or plan should live "inside
HyperDoc", treat that as a binding requirement, not as optional documentation.

### Priority wants: current Dock/state-model/editor cluster

For the current Dock / coachmark / state-model work, prioritize these wants in
HyperDoc-native form:

- edit markdown-page
- view and edit the state model
- inspectable/editable object with implementation evidence
- Codex handover prompts as chunks inside HyperDoc
- ingest sandbox artifacts such as `sandbox:/mnt/data/...` into HyperDoc rather
  than leaving them outside the durable graph
- add a frame/plugin-style story item or equivalent HyperDoc editing surface for
  concrete chrome fragments such as the Dock markup

These should appear as real HyperDoc artifacts, not just as prompt text.

## Authoritative Editable Models and Claim-Code Evidence

When a state machine, workflow model, UX contract, or capability grammar is part
of the task, the inspectable/editable HyperDoc object should be treated as the
authoritative model whenever practical.

Rules:

- do not leave the model only in prose, comments, or ad hoc code tables
- do not create a second disconnected "documentation model" if the user wants
  the HyperDoc object to *be* the model
- runtime behavior should interpret that model directly, or derive a narrow
  runtime representation from it
- the derivation boundary must be inspectable and test-covered
- each major claim in the model should have implementation evidence via
  claim-code relations to concrete source files, functions, views, and tests
- the user should be able to inspect both the model and its implementation
  evidence from HyperDoc

For UX/state-model work, the model view should answer at least:

- why is this state visible now?
- why did it recede or degrade?
- which compact affordance remains?
- which source code and tests implement this rule?

## Markdown Pages, Chunks, and Prompt Artifacts

Treat editable markdown pages, chunks, and prompt artifacts as first-class
HyperDoc authoring surfaces.

Rules:

- if the user wants to edit a markdown page, prefer making the markdown page
  itself editable rather than copying its content into detached prose
- Codex handover prompts that matter beyond the current turn should be stored as
  chunks inside HyperDoc, linked to their related pages/topics/claims
- do not leave important handover prompts only as chat output or sandbox files
- when a prompt evolves, update the canonical HyperDoc chunk instead of
  proliferating competing prompt copies

## Sandbox Artifact Ingestion Rule

When an answer produces an external artifact such as
`sandbox:/mnt/data/codex-dock-coachmark-handover-v2.html`, that artifact is not
durable enough by itself for this project.

Rules:

- bring important sandbox artifacts into HyperDoc as a page, chunk, source
  reference, or imported companion artifact
- link the sandbox-derived artifact to the relevant topic/page/claim structure
- if a sandbox artifact is superseded, preserve continuity with a shell or
  explicit successor relation rather than silently replacing the old path
- do not assume a sandbox link alone satisfies the artifact requirement for this
  repository

## Frame/Plugin Editing Boundary for Concrete Chrome

When the user wants to inspect or edit a concrete UI fragment (for example the
Dock HTML/DOM markup), provide a HyperDoc editing surface that can hold the
fragment as an inspectable/editable object.

Acceptable forms include a frame/plugin-style story item, chunk editor, or
equivalent narrow editing surface that keeps the fragment connected to source and
runtime evidence.

Rules:

- preserve the raw fragment as inspectable data
- link the fragment to its generating source file(s), CSS, JS, and tests
- make it possible to edit the fragment or its authoritative generating model
  without losing implementation traceability
- prefer chunk-level editing for local UI fragments instead of forcing every
  change through large page rewrites
