      ...
      (let ((old (car pair)) (new (cdr pair)))
        (setf result
                (with-output-to-string (out)
                  (loop with start = 0
                        for pos = (search old result :start2 start :test
                                          #'char-equal)
                        do (if pos
                               (progn
                                (write-string result out :start start :end pos)
                                (write-string new out)
                                (setf start (+ pos (length old))))
                               (progn
                                (write-string result out :start start)
                                (return))))))))))

(defun %html-decode-basic (string)
  (%replace-all-strings string
                        '(("&nbsp;" . " ") ("&amp;" . "&") ("&lt;" . "<")
                          ("&gt;" . ">") ("&quot;" . "\"") ("&#39;" . "'")
                          ("&rsquo;" . "'") ("&lsquo;" . "'")
                          ("&ldquo;" . "\"") ("&rdquo;" . "\"")
  ...
  (with-output-to-string (out)
    (loop with previous-space? = t
          ...
          do (cond
              ...
              (t (write-char char out) (setf previous-space? nil))))))
   ...
   (%html-decode-basic
    (with-output-to-string (out)
      (loop with inside-tag? = nil
            for char across string
            do (cond
                ((char= char #\LESS-THAN_SIGN) (setf inside-tag? t)
                 (write-char #\Space out))
                ((char= char #\GREATER-THAN_SIGN) (setf inside-tag? nil)
                 (write-char #\Space out))
                ((not inside-tag?) (write-char char out))))))))

(defun %blank-string-p (string) (every #'%whitespace-char-p string))
...
(defun %html-element-contents (tag html)
  (let ((results 'nil)
    ...
    (loop for start = (search start-token html :start2 position :test
                              #'char-equal)
          while start
          for open-end = (position #\GREATER-THAN_SIGN html :start start)
          while open-end
          for content-start = (1+ open-end)
          for
          end = (search end-token html :start2 content-start :test
                        #'char-equal)
          while
          end
          do (push (subseq html content-start end) results) (setf position
                                                                    (+ end
                                                                       (length
                                                                        end-token))))
    (nreverse results)))
...
(defun %text-of-html-elements (tag html)
  (remove-if #'%blank-string-p
             (mapcar #'%strip-html-tags (%html-element-contents tag html))))
  ...
  (let ((raw
         (with-output-to-string (out)
           (loop with last-hyphen? = nil
                 ...
                 do (cond
                     ((alphanumericp char) (write-char char out)
                      (setf last-hyphen? nil))
                     ((not last-hyphen?) (write-char #\HYPHEN-MINUS out)
                      (setf last-hyphen? t)))))))
    (string-trim "-" raw)))

(defun %first-sentence-title (text)
  (let ((dot (position #\FULL_STOP text)))
    (if dot
        (subseq text 0 dot)
        text)))
...
(defun %infer-shop3-topic-tags (text kind)
  (remove-duplicates
   (append (list :shop3 :planning kind)
           (when (search "HTN" text :test #'char-equal) (list :htn))
           (when (search "PDDL" text :test #'char-equal) (list :pddl))
           (when (search "Common Lisp" text :test #'char-equal)
             (list :common-lisp))
           (when (search "theorem-prover" text :test #'char-equal)
             (list :theorem-prover))
           (when (search "unifier" text :test #'char-equal) (list :unifier)))
   :test #'eq))

(defun %make-shop3-introduction-topic
       (index kind title text &key parent section source-url)
  (list :id (format nil "shop3-introduction/~2,'0D-~A" index (%slugify title))
        :title title :kind kind :parent parent :source-url source-url
        :source-section section :tags (%infer-shop3-topic-tags text kind) :text
        text))

(defun parse-shop3-introduction-topics
       (html
        &key (source-url *shop3-introduction-source-url*)
...
function performs no network access and no file writes."
  (let* ((headings (%text-of-html-elements "h2" html))
         ...
         (root-title (or (first headings) "SHOP3 Manual Introduction"))
         (topics
          (list
           (list :id root-id :title root-title :kind :source-chapter :parent
                 nil :source-url source-url :source-section "Introduction"
                 :tags '(:shop3 :manual :planning :htn) :text
                 "Topic root parsed from the SHOP3 manual Introduction chapter."))))
    (loop for paragraph in paragraphs
          for index from 1
          do (push
              (%make-shop3-introduction-topic index :concept
                                              (%paragraph-topic-title
                                               paragraph)
                                              paragraph :parent root-id
                                              :section
    ...
    (loop for feature in features
          ...
          do (push
              (%make-shop3-introduction-topic index :distinctive-characteristic
                                              (%feature-topic-title feature)
                                              feature :parent root-id :section
                                              "Distinctive characteristics list"
                                              :source-url source-url)
              topics))
    (nreverse topics)))
