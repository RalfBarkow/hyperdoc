...
(DEFUN RUN-CHANGE-TRACKING-TESTS ()
  (LET ((NAME (GENSYM "LIVE-ONLY-"))
        ...
        (PROGN
         (SETF (SYMBOL-FUNCTION NAME) (COMPILE NIL '(LAMBDA (X) (+ X 1)))
               (SYMBOL-FUNCTION UNREGISTERED)
                 (COMPILE NIL '(LAMBDA () :INVISIBLE)))
         (LET* ((CHANGE (WF:REGISTER-CHANGE NAME LOG))
                (OBSERVATION (WF:CHANGE-OBSERVATION CHANGE)))
           (ASSERT (EQUAL (LIST CHANGE) (WF:OUTSTANDING-CHANGES LOG)))
           (ASSERT (NULL (GETF OBSERVATION :SOURCE)))
           (ASSERT (EQ :UNOWNED-OR-UNAVAILABLE (GETF OBSERVATION :STATUS)))
           (ASSERT (EQ NAME (WF:CHANGE-OPERATION CHANGE)))
           (ASSERT (= 4 (FUNCALL (WF:CHANGE-FUNCTION CHANGE) 3)))
           (ASSERT
            (NOT
             (FIND UNREGISTERED (WF:OUTSTANDING-CHANGES LOG) :KEY
                   #'WF:CHANGE-OPERATION)))
           (ASSERT (EQ CHANGE (WF:REGISTER-CHANGE NAME LOG)))
           (ASSERT (= 1 (LENGTH (WF:OUTSTANDING-CHANGES LOG))))
           (SETF (SYMBOL-FUNCTION NAME) (COMPILE NIL '(LAMBDA (X) (+ X 2))))
           (ASSERT
            (NOT
             (GETF (WF:OBSERVE-CHANGE CHANGE) :RECORDED-DEFINITION-CURRENT-P)))
           (ASSERT (= 1 (LENGTH (WF:OUTSTANDING-CHANGES LOG))))
           (LET ((NEW (WF:REGISTER-CHANGE NAME LOG)))
             (ASSERT (NOT (EQ CHANGE NEW)))
             (ASSERT (EQUAL (LIST CHANGE NEW) (WF:OUTSTANDING-CHANGES LOG)))
             (ASSERT (EQ NEW (WF:REGISTER-CHANGE NAME LOG)))
             (ASSERT
              (GETF (WF:OBSERVE-CHANGE NEW) :RECORDED-DEFINITION-CURRENT-P)))
           (LET ((COPY (WF:OUTSTANDING-CHANGES LOG)))
             (SETF (CAR COPY) NIL)
             (ASSERT (EQ CHANGE (FIRST (WF:OUTSTANDING-CHANGES LOG)))))
           (LET ((OWNED (WF:REGISTER-CHANGE 'WF:PLAN-STATUS LOG)))
             (ASSERT
              (EQ :SOURCE-LOCATED
                  (GETF (WF:CHANGE-OBSERVATION OWNED) :STATUS)))
             (ASSERT (EQ :UNVERIFIED (WF:CHANGE-RECONSTRUCTION-STATUS OWNED)))
             (ASSERT (MEMBER OWNED (WF:OUTSTANDING-CHANGES LOG))))
           (ASSERT
            (EVERY
             (LAMBDA (ITEM)
               (EQ :UNVERIFIED (WF:CHANGE-RECONSTRUCTION-STATUS ITEM)))
             (WF:OUTSTANDING-CHANGES LOG)))))
      (FMAKUNBOUND NAME)
      (FMAKUNBOUND UNREGISTERED)))
  (FORMAT T
