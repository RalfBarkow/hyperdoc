...
(defun run-configured-address-contract-test nil
       (let*
             ((lookup
              ...
              (root (dreyeck/hyperspec:hyperspec-root-pathname)))
             ...
             (check (string= "Body/m_defmet.htm" relative-path)
                    "Relative DEFMETHOD path differs: ~S." relative-path)
             (check (string= "/hyperspec/Body/m_defmet.htm" url)
                    "HyperSpec URL is not local: ~S." url)
             ...
             (check
                    (null
                          (search "www.lispworks.com" (page-url page) :test
                                  (function char-equal)))
             ...
             (check asset
                    "Configured HyperSpec view has no static path asset.")
             (check (string= "/hyperspec/" (second asset))
                    "HyperSpec HTTP root differs: ~S." asset)
             (check
                    (equal (truename root)
                           (truename
...
(defun run-corpus-contract-test nil
       (let ((root (dreyeck/hyperspec:hyperspec-root-pathname)))
            (dolist
                    (relative-file
                                   (quote
                                          ("Front/index.htm"
                    ...
                    (check
                           (uiop:file-exists-p
                                               (merge-pathnames relative-file
                                                                root))
                           "Required HyperSpec file is absent: ~A."
                           relative-file))
            (dolist (relative-directory (quote ("Data/" "Issues/")))
                    (check
...
(defun run-missing-configuration-contract-test nil
       (let ((dreyeck/hyperspec::*hyperspec-root-override* nil))
            (let*
                  ((page (defmethod-hyperspec-page))
                   (view (content-view page))
                   (html (html-inspector-views:view-html view)))
                  ...
                  (check
                         (string= "/hyperspec/Body/m_defmet.htm"
                                  (page-url page))
                         "Missing configuration changed the local-only URL contract.")
                  (check
                         (null
                               (search "www.lispworks.com" html :test
                                       (function char-equal)))
                         "Missing configuration silently falls back to LispWorks.")))
       t)

(defun run-invalid-configuration-contract-test nil
       (let
            ...
            (let*
                  ...
                  (check
                         (search
                                 "does not contain a complete HyperSpec 7.0 corpus"
                                 html)
