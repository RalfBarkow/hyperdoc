...
(defun run-local-task-structure-smoke-test ()
  (let ((plan (make-pi-simulation-plan)))
    (assert-equal (pi-simulation-plan-task-ids)
                  (mapcar #'id-of (tasks-of plan))
                  "Plan must expose the expected local simulation task IDs")
    (dolist (task (tasks-of plan))
      (assert-true (preconditions-of task)
                   "Every simulation task must expose preconditions")
      (assert-true (effects-of task)
                   "Every simulation task must expose effects")
      (assert-true (status-of task)
                   "Every simulation task must expose a status")
      (assert-true (evidence-of task)
                   "Every simulation task must expose evidence"))))

(defun run-shop3-boundary-smoke-test ()
  (let* ((plan (make-pi-simulation-plan))
         (shop3-result (pi-simulation-shop3-plan-result plan))
         (steps (pi-simulation-shop3-plan-steps plan))
    ...
    (assert-true (typep plan 'hyperdoc/shop3:hyperdoc-htn-plan-result)
                 "Pi simulation plan must be a HyperDoc SHOP3 plan result")
