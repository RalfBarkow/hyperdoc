;;;; SHOP3-like Kioskbeerli plan-only planner objects
;;
;;;; Copyright (c) 2026
...
(defparameter *kioskbeerli-task-state-links*
  '(("declare-target" nil "declared" "Kioskbeerli target is declared.")
...
(defun %kioskbeerli-plan-tasks ()
  (list
   (%task "declare-target"
          "Declare physical Kioskbeerli target"
          :status "declared"
   ...
   (%task "verify-source-tree"
          ...
          :preconditions '("hauptsache/kioskbeerli source tree is available")
          :effects '("source tree can be inspected before build planning")
          :status "declared"
          :evidence-paths '("/Users/rgb/workspace/hauptsache/kioskbeerli"))
   (%task "verify-nix-lock"
          ...
          :status "declared"
          :evidence-paths '("/Users/rgb/workspace/hauptsache/kioskbeerli/flake.lock"))
   ...
   (%task "record-evidence"
          ...
          :status "missing-evidence"
          :evidence-paths '("kioskbeerli/trace.lisp"))
   (%task "mark-dashboard-status"
          ...
          :status "blocked"
          :evidence-paths '("hyperdoc/Kioskbeerli Dashboard.html"))))
...
(defun kioskbeerli-planner-run (&key (execution-mode :plan-only)
  ...
  (unless (eq execution-mode :plan-only)
    (error "Kioskbeerli planner only supports :PLAN-ONLY execution, not ~S."
           execution-mode))
  (let ((tasks (%kioskbeerli-plan-tasks)))
    ...
    (%attach-kioskbeerli-plan-run
     (make-instance 'kioskbeerli-plan-run
                    ...
                    :title "Kioskbeerli plan-only operational workflow"
