...
(defmethod views:text-representation ((dashboard kioskberrli-topic-dashboard))
  (format nil "~A: ~A"
          (title-of dashboard)
...
(defmethod views:text-representation
    ((manifest kioskberrli-fedwiki-asset-manifest))
  (format nil "~A assets" (page-slug-of manifest)))

(defmethod views:text-representation ((store kioskberrli-sqlite-store))
...
(defun %render-kioskberrli-object-list (objects &optional (empty "None."))
  (if objects
      (views:html
       (:ul
...
(views:defview 👀overview (dashboard kioskberrli-topic-dashboard)
  (let ((plan (planner-run-of dashboard))
    ...
    (views:html-view
     ...
     (views:html
      ...
      (%render-kioskberrli-object-list
       (kioskberrli-next-missing-evidence-tasks :run plan :trace trace)
      ...
      (:h4 "Asset links")
      (:ul
       (:li (:tt "http://localhost:3000/view/kioskberrli"))
       (:li (:tt "http://localhost:3000/assets/pages/kioskberrli/kioskberrli.asd")))
      (:h4 "SQLite status")
      (:p "Optional. Use "
          (:tt "(kioskberrli:open-or-create-sqlite-store ...)")
...
(views:defview 👀overview (run kioskbeerli-plan-run)
  (views:html-view
   ...
   (views:html
    ...
    (:h4 "Next executable steps")
    (%render-kioskberrli-object-list
     (kioskberrli-next-missing-evidence-tasks :run run)
     "No blocked or missing-evidence tasks.")
    (:h4 "Tasks")
    (%render-kioskberrli-object-list (tasks-of run)))))
...
(views:defview 👀overview (store kioskberrli-sqlite-store)
  (views:html-view
   :title "SQLite Store"
   :priority 1
   (views:html
    ...
    (:p "SQLite is optional. Core Kioskberrli objects remain available when this store is unavailable."))))

(views:defview 👀overview (manifest kioskberrli-fedwiki-asset-manifest)
  (views:html-view
