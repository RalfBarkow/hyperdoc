...
- why did it recede?
- how does Connect differ between introduction and active states?
...
Connect-specific requirements

Implement Connect according to this progressive-enhancement model.

- First encounter or newly relevant encounter:
  Dock/coachmark may expand and explain Connect.
- Active connect gesture:
...
- After repeated use or dismissal:
  the expanded presentation may recede, but Connect must still be available in compact form.
- Help/rediscovery:
  the richer teaching layer must be recoverable on demand.

Do not leave permanent onboarding prose in steady-state chrome. Explanatory copy is appropriate in introduction mode, not as always-visible toolbar text.
...
- the richer provider- or route-specific UI belongs in the pane body or in its own proper surface,
